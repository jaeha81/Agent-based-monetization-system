...
export async function runAutomatedVideoQa(contentId: number, videoBuffer?: Buffer): Promise<QaResult> {
  ...
  const checks = [
    ...
    { name: 'narration', passed: (!!process.env.GOOGLE_TTS_API_KEY || !!process.env.LOCAL_TTS_URL) && scriptLength >= 40, detail: `TTS 설정=${!!process.env.GOOGLE_TTS_API_KEY || !!process.env.LOCAL_TTS_URL}, 대본=${scriptLength}자` },
